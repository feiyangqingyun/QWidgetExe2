...
2. 字体放大：有些领导觉得设备节点的字体小了点，希望在整体基础上单独放大显示，因为他们平时主要用的就是这里，双击查看实时监控，当设备很多的时候，字体大的时候看的更清楚。
3. 码流节点：是否在设备节点下面还显示主码流和子码流两个子节点，这样可以分别打开不同的码流地址。默认开启，不需要可以关闭。
4. 分组节点：默认设备分组从录像机中取，意味着设备树列表永远是两三层固定的结构，有些场景希望是自定义多层级分组的结构，就需要开启，开启后从下面的分组节点中取。
