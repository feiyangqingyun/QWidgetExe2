...
17. 编码策略：ffmpeg内核专用，可选不做处理、自动转码、强转264、强转265，如果选择强转265，则不管视频源头是什么编码，统一转成265格式存储。开启编码后可以将设置的OSD等文字水印图形水印信息保存到文件中。
