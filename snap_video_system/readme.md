...
7. win系统如何查看设备名见本文档中 [打开本地摄像头](##13.7 打开本地摄像头)。
...
- DeviceVideoSmart专门演示如何拿到一路视频数据，然后视频数据自己去做人工智能算法识别，识别后的结果可以是一堆rect矩形区域，然后这些区域可以发回视频控件显示。这些计算后的区域信息也可以保存到视频文件或者重新推流出去（如果是推流需要额外购买推流组件）。
- **后期可能会定义一套私有协议，json格式数据，用于tcp/udp/http/mqtt等方式接收数据，然后插入日志记录和图文警情等。这样第三方只需要根据私有协议来就行。这样就不需要如何熟悉本软件源码，只需要会简单的使用即可。**
